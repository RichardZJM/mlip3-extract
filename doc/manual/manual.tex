...
%	To compile LAMMPS with the MLIP package:
%	\begin{enumerate}
%		\item create the \texttt{LAMMPS/lib/mlip/} directory;
%		\item copy \texttt{MLIP/Source/} to \texttt{LAMMPS/lib/mlip/};
%		\item remove \texttt{makefile} from \texttt{LAMMPS/lib/mlip/};
%		\item copy \texttt{Makefile}, \texttt{Makefile.lammps}, \texttt{Makefile.lammps.template} from		
%		\texttt{MLIP/Source/External/MLIP4LAMMPS/} to 
%		\texttt{LAMMPS/lib/mlip/};
%		\item build the static library \texttt{libmlip.a} in \texttt{LAMMPS/lib/mlip/} (just execute \texttt{make} in \texttt{LAMMPS/lib/mlip/}).
%		\item copy \texttt{MLIP/Source/External/MLIP4LAMMPS/USER-MLIP/} to 
%		\texttt{LAMMPS/src/USER-MLIP/};
%		\item install the MLIP package (in \texttt{LAMMPS/src/} execute \texttt{make yes-user-mlip});
%		\item build LAMMPS from \texttt{LAMMPS/src/} (execute \texttt{make serial} or \texttt{make mpi}).
%	\end{enumerate}
