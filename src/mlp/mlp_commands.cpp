...
#include "../sample_potentials.h"
                  ...
                  "    Settings given from the command line overrides the settings taken from the file.\n")
    {
        ...
        Settings settings;
        if (relaxation_settings_fnm != "")
            settings = LoadSettings(relaxation_settings_fnm);
        settings.Modify(opts);
        ...
        if (preselect)
        {
            ...
            if (!opts["threshold_save"].empty())
                settings["extrapolation_control:threshold_save"] = opts["threshold_save"];
            if (!opts["threshold_break"].empty())
                settings["extrapolation_control:threshold_break"] = opts["threshold_break"];
            if (!opts["save_extrapolative_to"].empty())
                settings["extrapolation_control:save_extrapolative_to"] = opts["save_extrapolative_to"];
        }
        settings["fit"] = "FALSE";
    ...
    BEGIN_COMMAND("select_add",
                  ...
                  "  --log=<string>: where to write the selection log, default="
                  " (none) \n")
    {
    ...
    BEGIN_UNDOCUMENTED_COMMAND("select",
                               ...
                               "  --save_selected_to=<string>: were to save selected configurations, default="
                               " (not saved)\n"
                               ...
                               "  --log=<int>: Where to write selection log. \"stdout\" and \"stderr\" corresponds to standard output streams; Default="
                               " (none)")
    {
        if (args.size() != 2)
        {
                                ...
                                for (A[1][2] = -M; A[1][2] <= M; A[1][2]++)
                                    for (A[2][0] = -M; A[2][0] <= M; A[2][0]++)
                                        for (A[2][1] = -M; A[2][1] <= M; A[2][1]++)
                                            ...
                                            {
                                                if (fabs(A.det()) * cfg_size < min_atoms)
                                                    continue;
                                                if (fabs(A.det()) * cfg_size > max_atoms)
                                                    continue;
                                                X = A * L * A.transpose();
                               ...
                               "Alternatively, applies the 3x3 integer matrix to the lattice\n")
    {
        ...
        else if (args.size() == 11)
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                try
                {
                    args_int[i / 3][i % 3] = stoi(args[i + 2]);
                }
                catch (invalid_argument)
                {
    ...
    )
    {
        ...
        const bool possibly_many_output_files = (opts["output_format"] == "vasp_poscar" || opts["output_format"] == "poscar") || (opts["output_format"] == "lammps_datafile");

        ifstream ifs(args[0], ios::binary);
        if (ifs.fail())
            ERROR("cannot open " + args[0] + " for reading");
        ...
        if (!possibly_many_output_files)
        {
            if (opts["append"] == "")
            {
                ...
                if (ofs.fail())
                    ERROR("cannot open " + args[1] + " for writing");
            }
            else
            {
                ofs.open(args[1], ios::binary | ios::app);
                if (ofs.fail())
                    ERROR("cannot open " + args[1] + " for appending");
            }
        }
        ...
        while (read_success)
        {
            ...
            if (opts["last"] == "")
            {
                ...
                if (opts["output_format"] == "vasp_poscar" || opts["output_format"] == "poscar")
                {
                    ...
                    {
                        if (count == 1)
                            cfg_last.WriteVaspPOSCAR(args[1] + "0", types_mapping);
                ...
                if (opts["output_format"] == "lammps_datafile")
                {
                    ...
                    {
                        if (count == 1)
                            cfg_last.WriteLammpsDatafile(args[1] + "0");
                               ...
                               "and APPENDs to the database db\n")
    {
        ...
        for (int i = 1; i < args.size(); i++)
        {
            ...
            if (opts["save_nonconverged"] != "")
                cfg.Save(ofs);
            else if (cfg.features["EFS_by"] != "VASP_not_converged")
                cfg.Save(ofs);
            else
                               ...
                               "   --correct_cell=<true/false>: orthogonolize supercell ")
    {
        ...
        if (args.size() != 3)
            ERROR("mlp fix_mindist: 3 arguments required\n");
                               ...
                               "   --mindist=<double>/<string>")
    {
        ...
        string lat_str = (opts["lattice"].empty()) ? to_string(2 * cutoff) + ",0.0,0.0,0.0," + to_string(2 * cutoff) + ",0.0,0.0,0.0," + to_string(2 * cutoff) : opts["lattice"];
                ...
                if (!opts["mindist"].empty())
                {
                    ...
                    // check the minimal distance between fixed atoms
                    {
                        ...
                        else
                        {
                            ...
                            {
                                Warning("Neighborhood mindist (" + to_string(cfg_tmp.MinDist()) + ") is smaller than target mindist (" + to_string(mindist) + ")!");
                                continue;
                            }
                        }
                    }
                    ...
                    while (mindist < target_mindist)
                    {
                        ...
                        {
                            Warning("Mindist correction failed: " + e.message);
                        }
                  ...
                  "mlp extract_problem potential.mtp train.cfg XTWX.bin XTWy.bin [options]:\n")
    {
        if (args.size() != 4)
        {
            cout << "mlp extract_problem: 4 arguments are required\n";
            return 1;
        }
        ...
        {
            sum_atoms += training_set[i].nbhs.size();
            for (int j = 0; j < training_set[i].nbhs.size(); j++)
                sum_neighs += training_set[i].nbhs[j].count;
        }

        int glob_neighs = sum_neighs;
        int glob_atoms = sum_atoms;

#ifdef MLIP_MPI
        MPI_Allreduce(&sum_neighs, &glob_neighs, 1, MPI_INT, MPI_SUM, mpi.comm);
        MPI_Allreduce(&sum_atoms, &glob_atoms, 1, MPI_INT, MPI_SUM, mpi.comm);
#endif
                  ...
                  "mlp calculate_loss potential.mtp train.cfg [options]:\n")
    {
        if (args.size() != 2)
        {
            cout << "mlp calculate_loss: 2 arguments are required\n";
            return 1;
        }
