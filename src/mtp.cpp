...
void MTP::ReadBasisOldFormat(ifstream& ifs)
{
    ...
    // reading min_val, max_val and initializing radial basis
    ...
    p_RadialBasis = new Basis_Shapeev(min_val, max_val, default_radial_basis_size);
...
void MTP::ReadRadialCoeffs(std::ifstream & ifs)
{
    ...
    if (radial_funcs_count <= 0 || radial_funcs_count > p_RadialBasis->size)
        ERROR("Error reading .mtp file");
    radial_coeffs.resize(radial_funcs_count, p_RadialBasis->size);
...
void MTP::WriteMTPBasis(ofstream& ofs)
{
    ...
    {
        ofs << "\t\t\t{";
        for (int j=0; j<p_RadialBasis->size-1; j++)
            ofs << radial_coeffs(i, j) << ", ";
        ofs << radial_coeffs(i, p_RadialBasis->size-1) << "}\n";
    }
...
void MTP::CalcBasisFuncs(const Neighborhood& nbh)
{
    ...
    for (int j=0; j<nbh.count; j++) {
        ...
        for (int i=0; i<alpha_index_basic_count; i++)
        {
            double* p_vals = &p_RadialBasis->vals[0];
            double val = 0.0;
            for (int rb_ind=0; rb_ind<p_RadialBasis->size; rb_ind++)
                val += radial_coeffs(alpha_index_basic[i][0], rb_ind) * p_vals[rb_ind];
            //double val = p_RadialBasis->vals[alpha_index_basic[i][0]];
...
void MTP::CalcBasisFuncsDers(const Neighborhood& nbh)
{
    ...
    for (int j=0; j<nbh.count; j++) {
        ...
        {
            double* p_vals = &p_RadialBasis->vals[0];
            double* p_ders = &p_RadialBasis->ders[0];
            double val = 0.0;
            double der = 0.0;
            for (int rb_ind=0; rb_ind<p_RadialBasis->size; rb_ind++)
            {
                val += radial_coeffs(alpha_index_basic[i][0], rb_ind) * p_vals[rb_ind];
                der += radial_coeffs(alpha_index_basic[i][0], rb_ind) * p_ders[rb_ind];
            }
            //double val = p_RadialBasis->vals[alpha_index_basic[i][0]];
            //double der = p_RadialBasis->ders[alpha_index_basic[i][0]];
    ...
    for (int i = 0; i < alpha_index_times_count; i++) {
        ...
        for (int j=0; j<nbh.count; j++) 
            for (int a=0; a<3; a++) 
                moment_ders(alpha_index_times[i][3], j, a) += 
                    val2 * (moment_ders(alpha_index_times[i][0], j, a) * val1 + 
                    val0 * moment_ders(alpha_index_times[i][1], j, a));
    }
        ...
        for (int j=0; j<nbh.count; j++)
            for (int a=0; a<3; a++)
            {
                ...
                if (se_ders_weights != nullptr)
                    out_grad_accumulator[i] += basis_ders(i, j, a) * se_ders_weights[j][a];
            }
    }
}
...
void MTP::CalcEFSGrads( Configuration& cfg,
                        ...
                        Array3D& out_str_cmpnts)
{
    ...
    for (int ind=0; ind<cfg.nbhs.size(); ind++) 
    {
        ...
        for (int i=0; i<alpha_count; i++)
            for (int j=0; j<nbh.count; j++)
                for (int a=0; a<3; a++) {
                    out_frc_cmpnts(nbh.my_ind, a, i) += basis_ders(i, j, a);
